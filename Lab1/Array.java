public class Array {
    ...
    boolean findPairWithSum(int sum) {
        if (load < 2/* Check if there are less than 2 elements */) 
            return false;
    
        // Using two pointers approach
        int left = 0;
        int right = load-1; /* Initialize right pointer to the last index */
    
        while (left < right/* Continue looping until left pointer is less than right */) {
            int currentSum = A[left]+A[right] /* Calculate sum of elements at left and right */;
            if (currentSum == sum/* Check if currentSum equals sum */) {
               return true; // Pair found
            } else if (currentSum < sum/* Check if currentSum is less than sum */) {
