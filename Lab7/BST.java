public class BST<T extends Comparable<T>> extends BT<T> {
	...
	public T delete(T item) {
		...
		if (current.left == null && current.right == null) {
			// insert your code
			if(currentIsLeftChild) parent.left = null;
		...
		// Case 2.2: If its child node is on the left
			...
			if (currentIsLeftChild) {
				// insert your code
				if(current != root){ // check i this is not the root if so
				parent.left = current.left; // change parent left to our left child
				current.left = null; // and remove that one
				}
				else{
					root = current.left;
					current.left = null;
